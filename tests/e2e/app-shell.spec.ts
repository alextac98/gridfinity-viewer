...
test("shows the homepage app launcher", async ({ page }) => {
  await page.goto("/");
  const appLauncher = page.getByRole("region", { name: "Choose a workspace" });

  await expect(
    page.getByRole("heading", { level: 1, name: "Gridfinity Center" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { level: 2, name: "Choose a workspace" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Sidebar minimized on home" }),
  ).toBeVisible();
  ...
  for (const appName of [
    ...
    await expect(
      appLauncher.getByRole("link", { name: appName }),
    ).toBeVisible();
  }

  await appLauncher.getByRole("link", { name: "Grid Generator" }).click();
...
test.describe("app shell", () => {
  ...
  test("renders the grid generator and persists grid settings", async ({
    page,
  }) => {
      ...
      .poll(async () =>
        page.evaluate(() => {
          ...
          return storedSettings
            ? JSON.parse(storedSettings).params.widthUnits
            : 0;
        }),
      )
      .toBe(4);
